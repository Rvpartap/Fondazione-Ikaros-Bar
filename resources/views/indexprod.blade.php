...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
	<head>
		...
		<title>Product List Page - Fondazione Ikaros Bar</title>
      ...
      <link rel="stylesheet" type="text/css" href="{{ url("/lib/Dancing-Script/font.css") }}" />
		...
		<link rel="stylesheet" type="text/css" href="{{ url("/css/indexprod_style.css") }}" />
   </head>
   	...
   	<div class="scroll-indicator">
		...
		<div class="container-sm">
			<header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-3 border-bottom">
      		...
      		<div class="col-md-3 text-end">
         		<button type="button" class="btn btn-dark mb-2 me-1 rounded-5" onclick="location.href='/prodotti/create'"><i class="fa-solid fa-cart-plus"></i>&#160; Add new product</button>
		...
		<div class="container-md">
			...
			<div class="animate__animated animate__fadeIn animate__delay-1s row mb-3 align-items-center">
   			<div class="col-md-8 float-start" style="font-size: 25px">&#160;<i class="fa-solid fa-file-pen"></i>&#160; The bar menù</div>
			...
			<table id="foodDataTable" class="animate__animated animate__fadeIn animate__delay-1s table shadow-lg table-striped">
				<thead class="users-table-head">
    				<tr class="table-dark align-items-center">
      				...
      				<th scope="col" class="text-break"><i class="fa-solid fa-image"></i>&#160; Picture</th>
      				<th scope="col" class="text-break"><i class="fa-solid fa-burger"></i>&#160; Product Name</th>
     	 				<th scope="col" class="text-break"><i class="fa-solid fa-euro-sign"></i>&#160; Price</th>
     	 				<th scope="col" class="text-break"><i class="fa-solid fa-circle-question"></i>&#160; Available Yes/No</th>
      				<th scope="col" class="text-break"><i class="fa-solid fa-gavel"></i>&#160; Take actions</th>
    				</tr>
  				</thead>
  				<tbody class="table-group-divider">
  				   @foreach($prodotti as $prodotti)
    					<tr>
      					<td scope="row" class="text-break"><span>{{$prodotti->id}}</span></td>
      					<td class="text-break"><img src="{{$prodotti->url_img}}" class="shadow" alt="Product-Picture" width="80px" style="pointer-events: none; border-radius: 8px;"></td>
      					<td class="text-break"><span>{{$prodotti->nome_prod}}</span></td>
      					<td class="text-break"><span>€ {{$prodotti->prezzo}}</span></td>
      					<td class="text-break"><span>{{$prodotti->active}}</span></td>
      					<td class="text-break">
      						<button class="btn btn-dark btn-sm mb-2 rounded-5 me-1" onclick="location.href='{{ route('prodotti.edit', $prodotti->id)}}'" type="submit">Edit the product</button>
      						<form action="{{ route('prodotti.destroy', $prodotti->id)}}" method="post" style="display: inline-block">
                     		@csrf
                        	@method('DELETE')
                        	<button class="btn btn-danger btn-sm rounded-5 mb-2" type="submit">Delete the product</button>
                  		</form>
      					</td>
    					</tr>
    				@endforeach
