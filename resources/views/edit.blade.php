...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
   ...
   <body class="container">
   	...
   	<button onclick="location.href='/students'" id="gohome" class="animate__animated animate__fadeInRight">&#160;&#160;<i class="fa-solid fa-gavel fa-lg"></i>&#160; Amministra gli Utenti &#160;</button>
        				...
        				<form method="post" action="{{ route('students.update', $student->id) }}">
              			...
              			@method('PATCH')
							<div class="form-floating mb-3">
         					<input type="text" class="form-control rounded-4" id="name" placeholder="Cognome e Nome" name="name" value="{{ $student->name }}"/>
            				<label for="name">&#160;<i class="fa-solid fa-user"></i> &#160; Cognome e Nome</label>
         				</div>
          				<div class="form-floating mb-3">
            				<input type="email" class="form-control rounded-4" id="email" placeholder="name@example.com" name="email" value="{{ $student->email }}"/>
            				<label for="email">&#160;<i class="fa-solid fa-at"></i> &#160; Indirizzo Email</label>
          				</div>
          				<div class="form-floating mb-3">
         					<select class="form-control form-select rounded-4" name="classe">
          				...
          				<div class="form-floating mb-3">
    		 	   			<input type="text" class="form-control rounded-4" id="password" name="password" value="{{ $student->password }}" placeholder="Password">
            				<label for="password">&#160;<i class="fa-solid fa-key"></i> &#160; Inserisci una Nuova Password</label>
          				</div>
          				...
          				@if ($errors->any())
    						...
    						@endif
  			 				<small class="text-break mb-0"><i class="fa-solid fa-bell fa-shake mt-3"></i>&#160; NOTE: Questa è una pagina ["Account Edit"] Amministrativa, ciò significa che puoi modificare tutti i dati di un' account, senza problemi.</small>
        				</form>
      			</div>
   			</div>
			</div>
