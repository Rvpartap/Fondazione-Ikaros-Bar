...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
	<head>
		...
		<title>Request Reset Password Page - Fondazione Ikaros Bar</title>
      ...
      <link rel="stylesheet" type="text/css" href="{{ url("/lib/Dancing-Script/font.css") }}" />
		...
		<link rel="stylesheet" type="text/css" href="{{ url("/css/email_style.css") }}" />
   ...
   <body class="container">
   	...
   	<button onclick="location.href='/login'" id="gologin" title="Back to Log In" class="animate__animated animate__fadeInLeft">&#160;<i class="fa-solid fa-reply fa-lg"></i>&#160;&#160;</button>
		<div class="py-3">
			<div class="modal modal-signin position-static pt-2 d-block form-signin w-100 m-auto animate__animated animate__fadeInUp" tabindex="-1" role="LoginForm" id="modalLogin">
  				<div class="modal-dialog" role="document">
					<div class="modal-content rounded-5 shadow-lg">
      		   	<div class="modal-header p-5 pb-4 border-bottom-0">
        			  		<h6 class="fw-bold mb-0 font-size"><i class="fa-solid fa-pen-to-square fa-lg"></i>&#160; Reimposta la Password (1)</h6>
      		   	</div>
      		  		<div class="modal-body p-5 pt-0">
        		   		<form method="POST" action="{{ route('password.email') }}">
        		   			@csrf
          		   		<div class="form-floating mb-3">
           			   		<input type="email" class="form-control rounded-4 @error('email') is-invalid @enderror" name="email" id="email" placeholder="Password" value="{{ old('email') }}" autocomplete="email" required="true">
           			   		<label for="email">&#160;<i class="fa-solid fa-at"></i>&#160; Inserisci Indirizzo Email<i style="color: red">*</i></label>
        			   		</div>
          			   	<button class="w-100 mb-2 btn btn-lg rounded-4 btn-primary" type="submit">Invia la richiesta</button>
          			  		<br>
                        ...
                        @enderror
                    		...
                    		@endif
        			  		...
        			  		<hr>
        			  		<p style="font-size: .875em;" class="mb-0" align="justify"><i class="fa-solid fa-bell fa-shake"></i>&#160;NOTE: Dopo aver cliccanto sul pulsante verrai riportato alla pagina <span class="badge bg-primary">HOME</span>, ciò significa che la richiesta è stata inviata. Controlla la tua posta e riceverai un'email a nome di Fondazione Ikaros Bar con il link per reimpostare la tua password.</p>
        			  		<hr>
