   	...
   	<!-- Header -->
		...
		<div class="container-sm">
			<header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-3 border-bottom">
   			<a href="/admin/home" class="mb-3 me-2 mb-md-0 text-dark text-decoration-none lh-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Torna alla Admin Home">
      		...
      		<div class="col-md-3 text-end">
					<button type="button" class="btn btn-dark me-1 rounded-5" onclick="location.reload();"><i class="fa-solid fa-rotate"></i>&#160; Reload page</button>
         		<a href="{{ route('logout') }}"><button type="button" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="btn btn-outline-danger rounded-5"><i class="fa-solid fa-right-from-bracket"></i>&#160; Exit</button></a>
   	...
   	<!-- Orders Table -->
		<div class="container-md">
            ...
            <div class="animate__animated animate__fadeIn animate__delay-1s alert alert-success alert-dismissible fade show" role="alert">
              	<div><i class="fa-solid fa-thumbs-up"></i>&#160; {{ session()->get('status') }}</div>
   		...
   		<table id="ordersDataTable" class="animate__animated animate__fadeIn animate__delay-1s table shadow-lg table-striped" style="max-width:100%">
  				<thead class="users-table-head">
    				<tr class="table-dark align-items-center">
        				...
        				<th scope="col" class="text-break"><i class="fa-solid fa-bag-shopping"></i>&#160; Order and quantity</th>
        				<th scope="col" class="text-break"><i class="fa-solid fa-euro-sign"></i>&#160; Total price</th>
        				<th scope="col" class="text-break"><i class="fa-solid fa-thumbs-up"></i>&#160; Confirm?</th>
        				<th scope="col" class="text-break"><i class="fa-solid fa-trash"></i>&#160; Delete?</th>
      			</tr>
    			</thead>
    			<tbody class="table-group-divider">
    				@foreach($orders as $order)
      				@php
        					$orderId = $order->id;
        					$personOrders = $orders->where('id', $orderId);
        					$totalPrice = $personOrders->sum('subtotale');
      				@endphp
      				<tr>
        					<td scope="row" class="text-break"><span>{{ $order->id }}</span></td>
        					<td class="text-break"><span>{{ $order->nome }}</span></td>
        					<td class="text-break"><span>{{ $order->classe }}</span></td>
        					<td class="text-break">
          					<span>
            					@foreach ($personOrders as $item)
              						{{ $item->prodotto }}: {{ $item->quantity }}<br>
            					@endforeach
          					</span>
        					</td>
        					<td class="text-break"><span>{{ $totalPrice }}</span></td>
        					<td id="id{{ $order->id }}" class="text-break"></td>
        						<script type="text/javascript">
          						if ({{ $order->conferma }} === 0) {
            						document.getElementById('id{{ $order->id }}').innerHTML = '<form method="post" action="{{ route('order.update', $order->id) }}"><input type="hidden" name="conferma" id="conferma" value="1" /> @csrf @method("PATCH") <button class="btn btn-success btn-sm rounded-5" type="submit"> Confirm </button></form> ';
          						} else {
            						document.getElementById('id{{ $order->id }}').innerHTML = '<p style="color: green;"><i class="fa-solid fa-thumbs-up"></i>&#160; Order Confirmed</p>';
          						}
        						</script>
        					<td class="text-break">
          					<form action="{{ route('order.destroy', $order->id)}}" method="post" style="display: inline-block">
            					@csrf
            					@method('DELETE')
            					<button class="btn btn-danger btn-sm mb-2 rounded-5" type="submit"> Delete </button>
          					</form>
        					</td>
      				</tr>
    				@endforeach
